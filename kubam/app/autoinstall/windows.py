...
class Windows(object):
    ...
    def build_boot_image(node, template, net_template):
        ...
        # Use fuse to mount the image, e.g: mount.exfat-fuse kube01.img kube01 
        o = call(["mount.exfat-fuse", new_image_name, new_image_dir])
        if not o == 0:
            return 1, "not able to run mount.exfat-fuse {0} {1}".format(new_image_name, new_image_dir)

        # Write the file over the existing file if it exists. Hack to over write file
        fw = new_image_dir + "/autounattend1.xml"
        fw_real = new_image_dir + "/autounattend.xml"
        try:
            with open(fw, 'w') as f:
                f.write(template)
        ...
        fw_real = new_image_dir + "/network.txt"
        try:
            with open(fw, 'w') as f:
