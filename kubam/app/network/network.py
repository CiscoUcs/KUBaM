...
@network.route(Const.API_ROOT2 + "/network", methods=['GET', 'POST', 'PUT', 'DELETE'])
@cross_origin()
def host_handler():
    if request.method == 'POST':
        j, rc = Network.create_hosts(request.json)
    elif request.method == 'PUT':
        j, rc = Network.update_hosts(request.json)
    elif request.method == 'DELETE':
        j, rc = Network.delete_hosts(request.json)
    else:
        j, rc = Network.list_hosts()
    return jsonify(j), rc
